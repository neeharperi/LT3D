...
class CenterHead(BaseModule):
    ...
    def get_task_detections(self, num_class_with_bg, batch_cls_preds,
        ...
        """
        predictions_dicts = []
        post_center_range = self.test_cfg['post_center_range']
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            if top_scores.shape[0] != 0:
                ...
                for score, label in zip(top_scores, top_labels):
                    ...
                    one_hot = 0.01 * torch.ones(sum(self.num_classes))
                    one_hot[label] = score 

                    total_scores.append(one_hot)

                total_scores = torch.stack(total_scores)
                total_labels = torch.arange(sum(self.num_classes))
                ...
                if nms is None:
                    if wide: 
                        selected = []
                        for class_id in torch.unique(total_labels):
                            id = class_id.item()
                            top_scores_id = total_scores[:, id].cuda()
                            ...
                            selected.append(select[:self.test_cfg.max_per_img])

                        selected = torch.cat(selected).unique()
                    ...
                    for class_id in torch.unique(total_labels):
                        ...
                        within_selected.append(select[:self.test_cfg.max_per_img])

                    within_selected = torch.cat(within_selected).unique()

                    keep = [True if i != "X" else False for i in nms]
                    top_scores_keep = torch.max(total_scores[:, keep], axis=1)[0].cuda()
                    across_selected = nms_bev(
                            boxes_for_nms,
                            top_scores_keep,
                            thresh=self.test_cfg['nms_thr'],
                            pre_max_size=self.test_cfg['pre_max_size'],
                    ...
                    within_labels = top_labels[within_selected]
                    across_labels = top_labels[across_selected]

                    selected = []
                    for i, within in enumerate(within_labels):
                        if nms[within] == "W":
                            selected.append(within_selected[i])

                    for i, across in enumerate(across_labels):
                        if nms[across] == "A":
                            selected.append(across_selected[i])

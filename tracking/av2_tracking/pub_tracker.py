...
class PubTracker(object):
  def __init__(self,  hungarian=False, max_age=0):
    ...
    self.AV2_CLS_VELOCITY_ERROR = AV2_CLS_VELOCITY_ERROR
  ...
  def step_centertrack(self, results, time_lag):
    if len(results) == 0:
      ...
      for det in results:
        ...
        if det['detection_name'] not in AV2_TRACKING_NAMES:
          continue 

        det['ct'] = np.array(det['translation'][:2])
        det['tracking'] = np.array(det['velocity'][:2]) * -1 * time_lag
        det['label_preds'] = AV2_TRACKING_NAMES.index(det['detection_name'])
        temp.append(det)
    ...
    max_diff = np.array([self.AV2_CLS_VELOCITY_ERROR[box['detection_name']] for box in results], np.float32)
